...
# ডিফল্ট স্টেট (টু-ওয়ে ফিউচার্স ও লাইভ চেকলিস্ট অনুযায়ী সাজানো)
DEFAULT_STATE = {
    "price": 0.0,
...
# =====================================================================
# SECTION 3: ১৭টি শক্তিশালী ক্যান্ডেলস্টিক প্যাটার্ন ডিটেক্টর (সম্পূর্ণ ত্রুটিমুক্ত)
# =====================================================================
...
<div class="max-w-md mx-auto">
    ...
    <!-- কনф্লুয়েন্স চেকলিস্ট প্যানেল (ডাইনামিক ইউআই) -->
    <div class="card p-4 mb-4 text-[11px]">
        <div class="flex justify-between mb-3 items-center">
            <h3 class="font-bold text-slate-700 text-xs">&#128737; প্রাতিষ্ঠানিক চেকলিস্ট</h3>
            <span id="check_mode" class="text-[9px] px-2 py-0.5 rounded font-black uppercase">LONG MODE</span>
        </div>
        <div class="grid grid-cols-2 gap-2 text-slate-600 font-semibold" id="checklist_container">
            <!-- জাভাস্ক্রিপ্ট দ্বারা ডাইনামিকভাবে রেন্ডার হবে -->
        </div>
    </div>
    ...
    async function update() {
        try {
            ...
            if (d.price > 0) {
                ...
                // --- ডাইনামিক প্রাতিষ্ঠানিক চেকলিস্ট রেন্ডারিং ---
                ...
                // SOL ১-ঘণ্টা EMA 200 এর ওপর ভিত্তি করে মোড ডিটেক্ট করা (বুলিশ = LONG, বেয়ারিশ = SHORT)
                const is_long_mode = conf.macro_bullish;
                let html = '';
                
                if (is_long_mode) {
                    check_mode.innerText = 'LONG MODE 📈';
                    check_mode.className = 'text-[9px] px-2 py-0.5 rounded font-black bg-green-50 text-green-700 border border-green-100';
                    
                    html += renderCheckItem('১ঘণ্টা ম্যাক্রো আপট্রেন্ড', conf.macro_bullish);
                    html += renderCheckItem('বিটকয়েন ট্রেন্ড আপ', conf.btc_bullish);
                    html += renderCheckItem('মূল্য VWAP এর ওপরে', conf.vwap_long);
                    html += renderCheckItem('১৫মি ইএমএ এলাইনমেন্ট', conf.ema_long);
                    html += renderCheckItem('১ঘণ্টা ম্যাকডি বুলিশ', conf.macd_long);
                    html += renderCheckItem('ভলিউম ব্রেকআউটের কনফার্ম', conf.volume_confirmed);
                    html += renderCheckItem('ক্রেতাদের চাপ (অর্ডার বুক)', conf.ob_long);
                    html += renderCheckItem('সবুজ ক্যান্ডেল প্যাটার্ন', conf.bull_signal);
                } else {
                    check_mode.innerText = 'SHORT MODE 📉';
                    check_mode.className = 'text-[9px] px-2 py-0.5 rounded font-black bg-red-50 text-red-700 border border-red-100';
                    
                    html += renderCheckItem('১ঘণ্টা ম্যাক্রো ডাউনট্রেন্ড', conf.macro_bearish);
                    html += renderCheckItem('বিটকয়েন ট্রেন্ড ডাউন', conf.btc_bearish);
                    html += renderCheckItem('মূল্য VWAP এর নিচে', conf.vwap_short);
                    html += renderCheckItem('১৫মি ইএমএ ডাউন-এলাইন', conf.ema_short);
                    html += renderCheckItem('১ঘণ্টা ম্যাকডি বেয়ারিশ', conf.macd_short);
                    html += renderCheckItem('ভলিউম ব্রেকআউটের কনফার্ম', conf.volume_confirmed);
                    html += renderCheckItem('বিক্রেতাদের চাপ (অর্ডার বুক)', conf.ob_short);
                    html += renderCheckItem('লাল ক্যান্ডেল প্যাটার্ন', conf.bear_signal);
                }
