...
# ইন-মেমোরি লোকাল ৫-মিনিট ক্যাশ গ্লোবাল ভ্যারিয়েবল
CANDLE_CACHE_SOL = []
CANDLE_CACHE_BTC = []
...
def merge_ohlcv_cache(cache_list, new_candles, max_len=2500):
    if not cache_list:
...
# =====================================================================
def bot_engine():
    global CANDLE_CACHE_SOL, CANDLE_CACHE_BTC
    ...
    while True:
        try:
            # --- ইনক্রিমেন্টাল ডাবল বুটস্ট্র্যাপ ক্যাশিং মেথড (SOL - 5m) ---
            if not CANDLE_CACHE_SOL:
                print("Bootstrapping SOL 5m candles cache (Batch 1)...")
                raw1 = exchange.fetch_ohlcv(SYMBOL, '5m', limit=1000)
                time.sleep(1)
                print("Bootstrapping SOL 5m candles cache (Batch 2)...")
                since1 = raw1[0][0] - (1000 * 5 * 60 * 1000)
                raw2 = exchange.fetch_ohlcv(SYMBOL, '5m', since=since1, limit=1000)
                time.sleep(1)
                print("Bootstrapping SOL 5m candles cache (Batch 3)...")
                since2 = raw2[0][0] - (500 * 5 * 60 * 1000)
                raw3 = exchange.fetch_ohlcv(SYMBOL, '5m', since=since2, limit=500)
                
                full_raw_sol = raw3 + raw2 + raw1
                CANDLE_CACHE_SOL = merge_ohlcv_cache([], full_raw_sol, max_len=2500)
            else:
                raw_sol = exchange.fetch_ohlcv(SYMBOL, '5m', limit=5)
                CANDLE_CACHE_SOL = merge_ohlcv_cache(CANDLE_CACHE_SOL, raw_sol, max_len=2500)
                
            df5 = pd.DataFrame(CANDLE_CACHE_SOL, columns=['t', 'o', 'h', 'l', 'c', 'v'])
            
            # --- ইনক্রিমেন্টাল ডাবল বুটস্ট্র্যাপ ক্যাশিং মেথড (BTC - 5m) ---
            if not CANDLE_CACHE_BTC:
                print("Bootstrapping BTC 5m candles cache...")
                raw_btc1 = exchange.fetch_ohlcv("BTC/USDT", '5m', limit=500)
                CANDLE_CACHE_BTC = merge_ohlcv_cache([], raw_btc1, max_len=500)
            else:
                raw_btc = exchange.fetch_ohlcv("BTC/USDT", '5m', limit=5)
                CANDLE_CACHE_BTC = merge_ohlcv_cache(CANDLE_CACHE_BTC, raw_btc, max_len=500)
                
            df_btc5 = pd.DataFrame(CANDLE_CACHE_BTC, columns=['t', 'o', 'h', 'l', 'c', 'v'])
            
            # --- ৫ মিনিটের ডাটা মেমোরি রিস্যাম্পলিং (Multi-Timeframe Generation) ---
            df5_temp = df5.copy()
            df5_temp['dt'] = pd.to_datetime(df5_temp['t'], unit='ms')
            df5_temp.set_index('dt', inplace=True)
            
            # বিটিসি রিস্যাম্পলিং
            df_btc5_temp = df_btc5.copy()
            df_btc5_temp['dt'] = pd.to_datetime(df_btc5_temp['t'], unit='ms')
            df_btc5_temp.set_index('dt', inplace=True)
            ...
            df15 = resample_df(df5_temp, '15m')
            df30 = resample_df(df5_temp, '30m')
            df1h = resample_df(df5_temp, '1h')
            df3h = resample_df(df5_temp, '3h')
            
            df_btc15 = resample_df(df_btc5_temp, '15m')
            ...
            # গ্লোবাল বিটিসি এভিজি ট্রু রেঞ্জ (ATR) ডিক্লেয়ারেশন (NameError প্রতিরোধে)
            ...
            cur.update({
                ...
                "analysis_15m": {
                    "rsi": round(r15, 1),
                    "ema20": round(e20_15m, 2),
                    "ema50": round(e50_15m, 2),
                    "vwap": round(vwap, 2),
                    "sig": "বুলিশ ✅" if p > e20_15m else "বেয়ারিশ ❌",
                    "pats": pats15
                },
                "analysis_30m": {
                    "rsi": round(r30, 1),
