...
# ইন-মেমোরি লোকাল ক্যাশ গ্লোবাল ভ্যারিয়েবলসমূহ (মাল্টি-টাইমফ্রেম আর্কিটেকচার)
CANDLE_CACHE_5M = []
CANDLE_CACHE_15M = []
CANDLE_CACHE_1H = []
CANDLE_CACHE_BTC = []
...
DEFAULT_STATE = {
    ...
    "analysis_5m": {"rsi": 0, "ema20": 0, "ema50": 0, "sig": "লোড হচ্ছে..."},
    "analysis_15m": {"rsi": 0, "ema20": 0, "ema50": 0, "vwap": 0, "sig": "লোড হচ্ছে...", "pats": []},
    "analysis_30m": {"rsi": 0, "ema20": 0, "ema50": 0, "sig": "লোড হচ্ছে..."},
    "analysis_1h": {"rsi": 0, "ema200": 0, "btc_price": 0, "sig": "লোড হচ্ছে...", "pats": []},
    "analysis_3h": {"rsi": 0, "sup": 0, "res": 0, "sig": "লোড হচ্ছে..."},
    "confluences": {
...
def get_advanced_pats(df):
    ...
    if not g2 and g1 and c1['c'] >= c2['o'] and c1['o'] <= c2['c']: p.append({"n": "বুলিশ এনগালфিং 📈", "t": "bull"})
    ...
    if not g2 and g1 and c1['o'] < c2['c'] and c1['c'] > (c2['o'] + c2['c']) / 2 and c1['c'] < c2['o']: p.append({"n": "পিয়ার্সিং LINE ⚡", "t": "bull"})
    if not g2 and g1 and c1['c'] < c2['o'] and c1['o'] > c2['c'] and b1 < b2: p.append({"n": "বুলিশ হারামি 🤰", "t": "bull"})
    if g1 and g2 and g3 and c1['c'] > c2['c'] and c2['c'] > c3['c'] and b1 > 0.3 * t1 and b2 > 0.3 * t2: p.append({"n": "থ্রি হোয়াইট সোলজারส์ 💂‍♂️", "t": "bull"})
...
# =====================================================================
def bot_engine():
    global CANDLE_CACHE_5M, CANDLE_CACHE_15M, CANDLE_CACHE_1H, CANDLE_CACHE_BTC
    ...
    while True:
        try:
            # --- ৫ মিনিট ক্যাশ বুটস্ট্র্যাপ এবং আপডেট (SOL) ---
            if not CANDLE_CACHE_5M:
                print("Bootstrapping SOL 5m candles...")
                raw_sol5 = exchange.fetch_ohlcv(SYMBOL, '5m', limit=100)
                CANDLE_CACHE_5M = merge_ohlcv_cache([], raw_sol5, max_len=100)
            else:
                raw_sol5 = exchange.fetch_ohlcv(SYMBOL, '5m', limit=5)
                CANDLE_CACHE_5M = merge_ohlcv_cache(CANDLE_CACHE_5M, raw_sol5, max_len=100)
            df5 = pd.DataFrame(CANDLE_CACHE_5M, columns=['t', 'o', 'h', 'l', 'c', 'v'])

            # --- ১৫ মিনিট ক্যাশ বুটস্ট্র্যাপ এবং আপডেট (SOL) ---
            if not CANDLE_CACHE_15M:
                print("Bootstrapping SOL 15m candles...")
                raw_sol15 = exchange.fetch_ohlcv(SYMBOL, '15m', limit=1000)
                CANDLE_CACHE_15M = merge_ohlcv_cache([], raw_sol15, max_len=1000)
            else:
                raw_sol15 = exchange.fetch_ohlcv(SYMBOL, '15m', limit=5)
                CANDLE_CACHE_15M = merge_ohlcv_cache(CANDLE_CACHE_15M, raw_sol15, max_len=1000)
            df15 = pd.DataFrame(CANDLE_CACHE_15M, columns=['t', 'o', 'h', 'l', 'c', 'v'])

            # --- ১ ঘণ্টা ক্যাশ বুটস্ট্র্যাপ এবং আপডেট (SOL) ---
            if not CANDLE_CACHE_1H:
                print("Bootstrapping SOL 1h candles...")
                raw_sol1h = exchange.fetch_ohlcv(SYMBOL, '1h', limit=500)
                CANDLE_CACHE_1H = merge_ohlcv_cache([], raw_sol1h, max_len=500)
            else:
                raw_sol1h = exchange.fetch_ohlcv(SYMBOL, '1h', limit=5)
                CANDLE_CACHE_1H = merge_ohlcv_cache(CANDLE_CACHE_1H, raw_sol1h, max_len=500)
            df1h = pd.DataFrame(CANDLE_CACHE_1H, columns=['t', 'o', 'h', 'l', 'c', 'v'])

            # --- বিটিসি ১৫মি ক্যাশ বুটস্ট্র্যাপ এবং আপডেট ---
            if not CANDLE_CACHE_BTC:
                print("Bootstrapping BTC 15m candles...")
                raw_btc = exchange.fetch_ohlcv("BTC/USDT", '15m', limit=100)
                CANDLE_CACHE_BTC = merge_ohlcv_cache([], raw_btc, max_len=100)
            else:
                raw_btc = exchange.fetch_ohlcv("BTC/USDT", '15m', limit=5)
                CANDLE_CACHE_BTC = merge_ohlcv_cache(CANDLE_CACHE_BTC, raw_btc, max_len=100)
            df_btc15 = pd.DataFrame(CANDLE_CACHE_BTC, columns=['t', 'o', 'h', 'l', 'c', 'v'])

            # --- ১৫মি ডাটা মেমোরি রিস্যাম্পলিং (৩০মি এবং ৩ঘণ্টা চার্ট জেনারেশন) ---
            df15_temp = df15.copy()
            df15_temp['dt'] = pd.to_datetime(df15_temp['t'], unit='ms')
            df15_temp.set_index('dt', inplace=True)
            
            def resample_df(df_in, rule):
                return df_in.resample(rule).agg({
                    't': 'first',
                    'o': 'first',
                    'h': 'max',
                    'l': 'min',
            ...
            df30 = resample_df(df15_temp, '30m')
            df3h = resample_df(df15_temp, '3h')
            
            p = df5['c'].iloc[-1]
            high_p = df5['h'].iloc[-1] 
            low_p = df5['l'].iloc[-1]
            
            btc_p = df_btc15['c'].iloc[-1]
            btc_e20 = ta.trend.ema_indicator(df_btc15['c'], 20).fillna(0).iloc[-1]
            btc_bullish = btc_p > btc_e20  
            btc_bearish = btc_p < btc_e20  
            btc_atr_15m = ta.volatility.average_true_range(high=df_btc15['h'], low=df_btc15['l'], close=df_btc15['c'], window=14).fillna(0).iloc[-1]
            ...
            # --- ৫ মিনিট চার্ট ইন্ডিকেটরস ---
            r5_series = ta.momentum.rsi(df5['c'], window=14).fillna(0)
            e20_5m_series = ta.trend.ema_indicator(df5['c'], 20).fillna(0)
            e50_5m_series = ta.trend.ema_indicator(df5['c'], 50).fillna(0)
            
            r5 = r5_series.iloc[-1]
            e20_5m = e20_5m_series.iloc[-1]
            e50_5m = e50_5m_series.iloc[-1]
            
            # --- ১৫ মিনিট চার্ট ইন্ডিকেটরস ---
            r15_series = ta.momentum.rsi(df15['c'], window=14).fillna(0)
            e20_15m_series = ta.trend.ema_indicator(df15['c'], 20).fillna(0)
            e50_15m_series = ta.trend.ema_indicator(df15['c'], 50).fillna(0)
            
            r15 = r15_series.iloc[-1]
            e20_15m = e20_15m_series.iloc[-1]
            e50_15m = e50_15m_series.iloc[-1]
            
            # --- ৩০ মিনিট চার্ট ইন্ডিকেটরস ---
            r30 = ta.momentum.rsi(df30['c'], window=14).fillna(0).iloc[-1]
            e20_30m = ta.trend.ema_indicator(df30['c'], 20).fillna(0).iloc[-1]
            e50_30m = ta.trend.ema_indicator(df30['c'], 50).fillna(0).iloc[-1]
            
            # --- ১ ঘণ্টা চার্ট ইন্ডিকেটরস ---
            ...
            # --- ৩ ঘণ্টা চার্ট ইন্ডিকেটরস এবং ডাইনামিক ৫০-পিরিয়ড সাপোর্ট/রেজিস্ট্যান্স ---
            r3h = ta.momentum.rsi(df3h['c'], window=14).fillna(0).iloc[-1]
            res_3h = df3h['h'].rolling(window=50).max().fillna(p).iloc[-1]
            sup_3h = df3h['l'].rolling(window=50).min().fillna(p).iloc[-1]
            
            p_closed = df15['c'].iloc[-2]
            r15_closed = r15_series.iloc[-2]
            e50_closed = e50_15m_series.iloc[-2]
            
            # ১৫মি চার্টে ডাইনামিক ২০-পিরিয়ড ব্রেকআউট লজিক (নিখুঁত ১৫মি ব্রেকআউট ফিল্টার)
            res_15m = df15['h'].rolling(window=20).max().fillna(p).iloc[-2]
            sup_15m = df15['l'].rolling(window=20).min().fillna(p).iloc[-2]
            breakout_15m_long = p_closed > res_15m
            breakout_15m_short = p_closed < sup_15m
            ...
            ema_long_alignment = p > e20_15m and p > e50_15m
            ema_short_alignment = p < e20_15m and p < e50_15m
            
            # এন্ট্রি নিতে ১৫মি রেজিস্ট্যান্স ব্রেকআউট বাধ্যতামূলক করা হয়েছে
            can_buy_long = (macro_bullish and 
                            ...
                            (mv > ms) and 
                            bull_signal and 
                            breakout_15m_long and 
                            cooldown_over)

            # ফিক্স ১: btc_bearish-এর পাশে থাকা nodding টাইপোটি সম্পূর্ণ সংশোধন করা হয়েছে
            can_buy_short = (macro_bearish and 
                             ...
                             (mv < ms) and 
                             bear_signal and 
                             breakout_15m_short and 
                             cooldown_over)

            # --- ৫ মিনিট ক্লোজড ও ১৫ মিনিট ক্লোজড ডাইনামিক আর্লি-ওয়ার্নিং এক্সিট লজিক ---
            p_5m_closed = df5['c'].iloc[-2]
            r5_closed = r5_series.iloc[-2]
            e50_5m_closed = e50_5m_series.iloc[-2]
            
            long_smart_sell = (p_5m_closed < e50_5m_closed) or (r5_closed > 78) or (p_closed < e50_closed) or (r15_closed > 78)
            short_smart_sell = (p_5m_closed > e50_5m_closed) or (r5_closed < 22) or (p_closed > e50_closed) or (r15_closed < 22)
            ...
            if in_pos:
                ...
                if p > e200:
                    ...
                    if p < e20_15m or p < e50_15m:
                        ema_gap = max(e20_15m, e50_15m) - p
            ...
            # ৫টি সুনির্দিষ্ট লাইভ এক্সিট চেকলিস্ট ডাটা ডিকশনারি পাস করা
            if position_type == "LONG":
                exit_conditions = {
            ...
            cur.update({
                ...
                "analysis_5m": {
                    "rsi": round(r5, 1),
                    "ema20": round(e20_5m, 2),
                    "ema50": round(e50_5m, 2),
                    "sig": "বুলিশ ✅" if p > e20_5m else "বেয়ারিশ ❌"
                },
                "analysis_15m": {
                    "rsi": round(r15, 1),
                    "ema20": round(e20, 2),
                    "ema50": round(e50_15m, 2),
                    "vwap": round(vwap, 2),
                ...
                "analysis_30m": {
                    "rsi": round(r30, 1),
                    "ema20": round(e20_30m, 2),
                    "ema50": round(e50_30m, 2),
                    "sig": "বুলিশ ✅" if p > e20_30m else "বেয়ারিশ ❌"
                },
                "analysis_1h": {
                    "rsi": round(r1h, 1),
                    "ema200": round(e200, 2),
                ...
                "analysis_3h": {
                    ...
                    "sup": round(sup_3h, 2),
                    "res": round(res_3h, 2),
                    "sig": "বুলিশ ✅" if p > sup_3h else "বেয়ারিশ ❌"
                }
            })
...
<div class="max-w-md mx-auto">
    ...
    <div class="card p-4 mb-4 text-[11px]">
        <div class="flex justify-between mb-3 items-center">
            <h3 class="font-bold text-slate-700 text-xs">&#128202; ৫ মিনিট বিশ্লেষণ</h3>
            <span id="s5m" class="font-bold px-2 py-0.5 rounded text-[10px]">WAIT</span>
        </div>
        <div class="grid grid-cols-2 text-slate-500 font-medium">
            <span>RSI: <b id="r5">0</b></span>
            <span>EMA 20: <b id="e20_5m">0</b></span>
            <span>EMA 50: <b id="e50_5m">0</b></span>
        </div>
    </div>
    ...
    <!-- ৩০ মিনিট বিশ্লেষণ প্যানেল -->
    <div class="card p-4 mb-4 text-[11px]">
        <div class="flex justify-between mb-3 items-center">
            <h3 class="font-bold text-slate-700 text-xs">&#128202; ৩০ মিনিট বিশ্লেষণ</h3>
            <span id="s30" class="font-bold px-2 py-0.5 rounded text-[10px]">WAIT</span>
        </div>
        <div class="grid grid-cols-2 text-slate-500 font-medium">
            <span>RSI: <b id="r30">0</b></span>
            <span>EMA 20: <b id="e20_30m">0</b></span>
            <span>EMA 50: <b id="e50_30m">0</b></span>
        </div>
    </div>
    ...
    <!-- ৩ ঘণ্টা বিশ্লেষণ প্যানেল -->
    <div class="card p-4 mb-4 text-[11px]">
        <div class="flex justify-between mb-3 items-center">
            <h3 class="font-bold text-slate-700 text-xs">&#128202; ৩ ঘণ্টা বিশ্লেষণ (S/R)</h3>
            <span id="s3h" class="font-bold px-2 py-0.5 rounded text-[10px]">WAIT</span>
        </div>
        <div class="grid grid-cols-2 text-slate-500 font-medium">
            <span>RSI: <b id="r3h">0</b></span>
            <span>সাপোর্ট (S): <b id="sup_3h">0</b></span>
            <span>রেজিস্ট্যান্স (R): <b id="res_3h">0</b></span>
        </div>
    </div>
    ...
    async function update() {
        try {
            ...
            if (d.price > 0) {
                ...
                document.getElementById('e20_5m').innerText = '$' + d.analysis_5m.ema20;
                document.getElementById('e50_5m').innerText = '$' + d.analysis_5m.ema50;
                const s5m = document.getElementById('s5m'); 
                s5m.innerText = d.analysis_5m.sig;
                if (d.analysis_5m.sig.includes('বুলিশ')) {
                    s5m.className = 'font-bold px-2 py-0.5 rounded text-[10px] bg-green-50 text-green-700 border border-green-200';
                } else {
                    s5m.className = 'font-bold px-2 py-0.5 rounded text-[10px] bg-red-50 text-red-700 border border-red-200';
                }
                ...
                // ৩০ মিনিট রেন্ডারিং
                document.getElementById('r30').innerText = d.analysis_30m.rsi; 
                document.getElementById('e20_30m').innerText = '$' + d.analysis_30m.ema20;
                document.getElementById('e50_30m').innerText = '$' + d.analysis_30m.ema50;
                const s30 = document.getElementById('s30'); 
                s30.innerText = d.analysis_30m.sig;
                if (d.analysis_30m.sig.includes('বুলিশ')) {
                    s30.className = 'font-bold px-2 py-0.5 rounded text-[10px] bg-green-50 text-green-700 border border-green-200';
                } else {
                    s30.className = 'font-bold px-2 py-0.5 rounded text-[10px] bg-red-50 text-red-700 border border-red-200';
                }
                ...
                // ৩ ঘণ্টা রেন্ডারিং
                document.getElementById('r3h').innerText = d.analysis_3h.rsi; 
                document.getElementById('sup_3h').innerText = '$' + d.analysis_3h.sup;
                document.getElementById('res_3h').innerText = '$' + d.analysis_3h.res;
                const s3h = document.getElementById('s3h'); 
                s3h.innerText = d.analysis_3h.sig;
                if (d.analysis_3h.sig.includes('বুলিশ')) {
                    s3h.className = 'font-bold px-2 py-0.5 rounded text-[10px] bg-green-50 text-green-700 border border-green-200';
                } else {
                    s3h.className = 'font-bold px-2 py-0.5 rounded text-[10px] bg-red-50 text-red-700 border border-red-200';
                }
