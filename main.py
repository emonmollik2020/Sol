...
# =====================================================================
# SECTION 2: বটের তথ্য সংরক্ষণ ও লোড করার লজিক (State Management)
# =====================================================================
def save_state(d):
    """বটের বর্তমান তথ্য JSON ফাইলে সেভ করে"""
    with open(STATE_FILE, "w") as f:
        json.dump(d, f)


def load_state():
    """ফাইল থেকে তথ্য লোড করে, ফাইল না থাকলে ডিফল্ট তথ্য সেট করে"""
    if not os.path.exists(STATE_FILE):
        return {
            ...
            "sl_level": 0.0,
            "tp_level": 0.0,
            "analysis_1m": {"rsi": 0, "ema": 0, "sig": "Loading", "pats": []},
            "analysis_3m": {"rsi": 0, "macd": 0, "sig": "Loading", "pats": []},
...
def bot_engine():
    ...
    while True:
        try:
            ...
            # শর্তাবলী: মূল্য EMA20 এর উপরে, RSI ৪০ থেকে ৬৫ এর ভেতরে, MACD বুলিশ ক্রসওভার, বুলিশ প্যাটার্ন ও কুলডাউন সম্পন্ন
            ...
            smart_sell = r1 > 75 or bear_signal  # RSI অতিরিক্ত ওভারবট বা ৩ মিনিটে বেয়ারিশ ক্যান্ডেলস্টিক
...
@app.route('/api/data')
def api():
    """বটের বর্তমান তথ্য JSON ফরম্যাটে প্রদান করে"""
...
if __name__ == "__main__":
    app.run(host='0.0.0.0', port=int(os.environ.get('PORT', 8080)))            cur = load_state()
            l_pnl = ((p/entry_p)-1)*100 if in_pos else 0.0
            l_val = (100.0/entry_p*p)-100.0 if in_pos else 0.0
