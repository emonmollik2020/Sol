...
def get_advanced_pats(df):
    ...
    if g3 and b2 < (b3 * 0.3) and not g1 abuses and c1['c'] < (c3['o'] + c3['c']) / 2: p.append({"n": "ইভনিং স্টার 🌅", "t": "bear"})
...
# =====================================================================
# SECTION 3.5: টাইম এস্টিমেটর ফরমেট হেল্পার
# =====================================================================
    ...
    while True:
        try:
            ...
            if in_pos:
                ...
                atr_15m = ta.volatility.average_true_range(high=df15['h'], low=df15['l'], close=df15['c'], window=14).fillna(0).iloc[-1]
                ...
                # সোলানা আপট্রেন্ড জোনে থাকলে সম্ভাব্য লং এন্ট্রির বাধাগুলো বিশ্লেষণ করা হচ্ছে
                if p > e200:
                    if btc_p < btc_e20:
                    ...
                    if p < vwap:
                        vwap_gap = vwap - p
                        t_vwap = (vwap_gap / max(0.01, atr_15m)) * 900
                        
                    if p < e20 or p < e50:
                        ema_gap = max(e20, e50) - p
                        t_ema = (ema_gap / max(0.01, atr_15m)) * 900
                        
                    if mv < ms:
                        macd_gap = ms - mv
                        t_macd = (macd_gap / 0.05) * 3600
                        
                # সোলানা ডাউনট্রেন্ড জোনে থাকলে সম্ভাব্য শর্ট এন্ট্রির বাধাগুলো বিশ্লেষণ করা হচ্ছে
                else:
                    if btc_p > btc_e20:
                ...
                # সমন্বিত সর্বোচ্চ অপেক্ষার সময় নির্ধারণ
            ...
            cur.update({
                ...
                "estimated_time": est_str, # ড্যাশবোর্ডে সিরিয়াল এস্টিমেটেড টাইম ডাইনামিক আপডেট
                "analysis_15m": {
                    "rsi": round(r15, 1),
                    "ema20": round(e20, 2),
                    "ema50": round(e50, 2),
...
<div class="max-w-md mx-auto">
    ...
    <div class="card p-6 mb-4 text-center">
        ...
        <!-- এস্টিমেটেড টাইম ড্যাশবোর্ড প্যানেল (স্ক্রিনশট ডিজাইনের সাথে সুসংগত) -->
    ...
    async function update() {
        try {
            ...
            if (d.price > 0) {
                ...
                document.getElementById('est').innerText = d.estimated_time; // ফ্রন্টএন্ডে রিয়েল-টাইম টাইমিং রেন্ডার
